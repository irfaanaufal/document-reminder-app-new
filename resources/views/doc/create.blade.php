        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow-sm sm:rounded-lg border border-gray-200 dark:border-zinc-800">
                <div class="p-6 text-gray-900 dark:text-zinc-100">
                    ...
                    <form method="POST" action="{{ route('doc.store') }}" enctype="multipart/form-data" id="documentForm" class="space-y-6">
                        @csrf
                        ...
                        <div class="max-w-full">
                            ...
                            <select id="jenis_dokumen" name="jenis_dokumen" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required onchange="showRelevantForm()">
                                ...
                                @foreach ($documentTypes as $documentType)
                                    <option value="{{ $documentType->id }}" data-tipe-form="{{ $documentType->tipe_form }}" @selected(old('jenis_dokumen') == $documentType->id)>{{ $documentType->nama_jenis }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div id="form-sertifikat" class="space-y-6 hidden">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="nama_dokumen" name="nama_dokumen" type="text" class="mt-1 block w-full" :value="old('nama_dokumen')" placeholder="Masukkan nama sertifikat" />
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="tanggal_terbit" name="tanggal_terbit" type="date" class="mt-1 block w-full" :value="old('tanggal_terbit')" />
                                </div>
                                <div>
                                    <x-input-label for="tanggal_expired" :value="__('Tanggal Expired')" />
                                    <x-text-input id="tanggal_expired" name="tanggal_expired" type="date" class="mt-1 block w-full" :value="old('tanggal_expired')" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="form-wajib-lapor-tahunan" class="space-y-6 hidden">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="nama_dokumen_wlt" name="nama_dokumen" type="text" class="mt-1 block w-full" :value="old('nama_dokumen')" placeholder="Masukkan nama wajib lapor" />
                                </div>
                                <div>
                                    <x-input-label for="tahun_laporan" :value="__('No Pelaporan')" />
                                    <x-text-input id="tahun_laporan" name="no_dokumen" type="text" class="mt-1 block w-full" :value="old('no_dokumen')" placeholder="Masukkan nomor pelaporan" />
                                </div>
                                <div>
                                    <x-input-label for="instansi_tujuan" :value="__('Instansi Tujuan')" />
                                    <x-text-input id="instansi_tujuan" name="penerbit_tujuan" type="text" class="mt-1 block w-full" :value="old('penerbit_tujuan')" placeholder="Masukkan nama instansi" />
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="tanggal_terbit_wlt" name="tanggal_terbit" type="date" class="mt-1 block w-full" :value="old('tanggal_terbit')" />
                                </div>
                                <div>
                        ...
                        <div class="bg-gray-50 dark:bg-zinc-800/50 p-6 rounded-lg border border-gray-200 dark:border-zinc-700 space-y-6">
                            ...
                            <input type="hidden" name="pic_nama" id="primary_pic_nama" value="{{ old('pic_nama') }}">
                            <input type="hidden" name="pic_telpon" id="primary_pic_telpon" value="{{ old('pic_telpon') }}">
                            ...
                            <div class="pt-6 border-t border-gray-200 dark:border-zinc-700">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-text-input name="pic_external_nama" type="text" class="mt-1 block w-full" :value="old('pic_external_nama')" />
                                    </div>
                                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                            <div>
                                ...
                                <select id="reminder_bulan_global" name="reminder_bulan" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    <option value="1" @selected(old('reminder_bulan') == '1')>1 bulan sebelum expired</option>
                                    <option value="3" @selected(old('reminder_bulan') == '3')>3 bulan sebelum expired</option>
                                    <option value="6" @selected(old('reminder_bulan') == '6')>6 bulan sebelum expired</option>
                                    <option value="9" @selected(old('reminder_bulan') == '9')>9 bulan sebelum expired</option>
                                    <option value="12" @selected(old('reminder_bulan') == '12')>12 bulan sebelum expired</option>
                                </select>
                            </div>

                            <div>
                                <x-input-label for="attachment" :value="__('Lampiran Dokumen (PDF/PNG/JPG/JPEG, Maks 3MB)')" />
                                <input id="attachment" name="attachment" type="file" accept=".pdf,.png,.jpg,.jpeg,application/pdf,image/png,image/jpeg" class="mt-1 block w-full text-sm text-gray-500 dark:text-zinc-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-zinc-100 dark:file:bg-zinc-800 file:text-zinc-700 dark:file:text-zinc-300 hover:file:bg-zinc-200 dark:hover:file:bg-zinc-700 transition-colors" required>
                            </div>
                        </div>
