    ...
    <div class="py-6">
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <form action="{{ route('doc_type.update', $documentType) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PATCH')
                    <div>
                        <label for="nama_jenis" class="block text-sm font-medium text-gray-700 dark:text-zinc-300">Nama Jenis</label>
                        <input id="nama_jenis" name="nama_jenis" type="text" value="{{ old('nama_jenis', $documentType->nama_jenis) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100" required>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100" required>
                            ...
                            <option value="deactive" @selected(old('status', $documentType->status) === 'deactive')>Deactive</option>
                        </select>
                    </div>
